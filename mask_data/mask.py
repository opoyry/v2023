...
# This script uses Faker to mask real strings and monetary values with generated fake data for the bank statement and manual journal entries.
...
def UploadToS3():
    ...
    s3.Bucket(BUCKET).upload_file(
        outputFile, "snowflake/gl_sample_data/manual_journal/manual_journal.parquet"
    )  # s3://essaimdev/snowflake/gl_sample_data/manual_journal/
    outputFile = os.environ["masked.bank_statement.outputFile"]
    s3.Bucket(BUCKET).upload_file(
        outputFile, "snowflake/gl_sample_data/bank_statement/bank_statement.parquet"
    )  # s3://essaimdev/snowflake/gl_sample_data/manual_journal/

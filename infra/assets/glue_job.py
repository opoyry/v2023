...
class Utils:
    ...
    def get_data_from_s3(self):
        ...
        for i in range(len(response)):
            event_payload = json.loads(response[i]["CloudTrailEvent"])[
                "requestParameters"
            ]["eventPayload"]
    ...
    def send_notification(self, message):
        ...
        resp = http.request(
            method="POST", url=_url, body=json.dumps(_msg).encode("utf-8")
        )
...
wr.s3.to_parquet(
    df=coffee_data,
    path=f's3://{utils.bucket_name.replace("raw", "processed")}/coffee_data.parquet',
)

...
{%- if not dbt_constraints.unique_constraint_exists(table_relation, column_names, lookup_cache) -%}
        ...
        {%- if dbt_constraints.have_ownership_priv(table_relation, verify_permissions, lookup_cache) -%}
            ...
            {%- set query -%}
            -- DOES NOT WORK IN DUCKDB: ALTER TABLE {{ table_relation }} ADD CONSTRAINT {{ constraint_name }} PRIMARY KEY ( {{ columns_csv }} )
            {%- endset -%}
...
where table_name = '{{ table_relation }}'
...
{%- set table_privileges = duckdb__lookup_table_privileges(table_relation, lookup_cache) -%}
